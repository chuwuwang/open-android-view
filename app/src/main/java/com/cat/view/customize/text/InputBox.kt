...
class InputBox @JvmOverloads constructor(
    ...
    companion object {
        ...
        private const val TAG = "InputBox"
    ...
    private val typedArray = context.obtainStyledAttributes(attrs, R.styleable.InputBox, defStyleAttr, 0)

    init {
        try {
            size = typedArray.getInteger(R.styleable.InputBox_input_size, DEFAULT_CODE_SIZE)
            margin = typedArray.getDimension(R.styleable.InputBox_input_margin, DEFAULT_CODE_MARGIN)
            itemBackground = typedArray.getDrawable(R.styleable.InputBox_input_background)
            itemCursorDrawable = typedArray.getDrawable(R.styleable.InputBox_input_cursorDrawable)
            itemCursorVisible = typedArray.getBoolean(R.styleable.InputBox_input_cursorVisible, true)
            if (itemCursorDrawable == null) {
                val color = ContextCompat.getColor(context, android.R.color.darker_gray)
                val gradientDrawable = GradientDrawable()
                gradientDrawable.setColor(color)
                gradientDrawable.setSize(3, 0)
                itemCursorDrawable = gradientDrawable
            }
        } finally {
            typedArray.recycle()
        }
